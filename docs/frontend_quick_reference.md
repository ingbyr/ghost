...
### 远程Host定时刷新功能
```
1. 在RemoteHost编辑器中可以设置定时刷新间隔（1小时、4小时、8小时、24小时）
2. 定时刷新功能仅对远程Host分组生效
3. 设置定时刷新后，系统会在指定时间间隔自动获取远程URL内容
4. 只有启用的远程Host分组才会在定时刷新时应用到系统Hosts文件
5. 刷新间隔可以选择：禁用（0秒）、1小时（3600秒）、4小时（14400秒）、8小时（28800秒）、24小时（86400秒）
```
